...
void WampEventCallbacks(const uint64_t& in_subscriptionId, const JsonProvider& in_rJson)
{
	...
	FString jsonString = in_rJson.GetJsonString().c_str();
	TSharedRef<TJsonReader<>> Reader = TJsonReaderFactory<>::Create(*jsonString);
	if (!FJsonSerializer::Deserialize(Reader, ueJsonObject) || !ueJsonObject.IsValid())
	{
		UE_LOG(LogAkWaapiClient, Log, TEXT("Unable to deserialize a JSON object from the string : %s"), *jsonString);
		return;
	}
...
bool FAkWaapiClient::AttemptConnection()
{
	...
	{
		bConnected = m_Impl->m_Client.Connect(TCHAR_TO_ANSI(*AkSettingsPerUser->WaapiIPAddress), AkSettingsPerUser->WaapiPort);
	}
	else
	{
